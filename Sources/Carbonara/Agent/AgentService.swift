...
final class AgentService {
    ...
    /// Resolved `claude` binary, or nil when Claude Code isn't installed. Set off-main at init.
    private var claudeCodeBinary: URL?
    ...
    private func resolveClaudeCode() {
        Task { [weak self] in
            let binary = await Task.detached(priority: .utility) { ClaudeCodeLocator.resolve() }.value
            self?.claudeCodeBinary = binary
        }
    }

    var usesClaudeCode: Bool { claudeCodeBinary != nil }
    ...
    private func selectClient() -> (any AgentClient)? {
        if let binary = claudeCodeBinary, let editor {
            AppState.shared.startMCPService()
            let sessionID = currentSession?.claudeCodeSessionID
            let currentID = currentSessionId
            return ClaudeCodeClient(
                binary: binary,
                mcpEndpoint: "http://127.0.0.1:\(MCPService.port)/mcp",
                workingDirectory: editor.projectURL?.deletingLastPathComponent()
                    ?? FileManager.default.homeDirectoryForCurrentUser,
                resumeSessionID: sessionID,
                onSessionID: { [weak self] id in
                    Task { @MainActor in self?.storeClaudeCodeSessionID(id, forSession: currentID) }
                }
            )
        }
    ...
    private func storeClaudeCodeSessionID(_ id: String, forSession sessionID: UUID?) {
        guard let sessionID, let idx = sessions.firstIndex(where: { $0.id == sessionID }) else { return }
        sessions[idx].claudeCodeSessionID = id
    }
