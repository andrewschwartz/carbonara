...
struct ChatSession: Codable, Identifiable {
    ...
    var isOpen: Bool
    /// Claude Code session id for `--resume`; nil until the first turn assigns one.
    ...
    init(id: UUID = UUID(), title: String = "New chat", messages: [AgentMessage] = [], isOpen: Bool = true) {
        ...
        self.isOpen = isOpen
        self.claudeCodeSessionID = nil
    }

    private enum CodingKeys: String, CodingKey { case id, title, updatedAt, messages, isOpen, claudeCodeSessionID }
    ...
    init(from decoder: Decoder) throws {
        ...
        self.claudeCodeSessionID = try c.decodeIfPresent(String.self, forKey: .claudeCodeSessionID)
    }
}
