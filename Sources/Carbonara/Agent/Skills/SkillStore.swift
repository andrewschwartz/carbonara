...
/// Reads skills from `~/.carbonara/skills/` — the single source of truth.
@Observable
@MainActor
final class SkillStore {
    ...
    nonisolated static var directory: URL {
        FileManager.default.homeDirectoryForCurrentUser
            .appendingPathComponent(".carbonara/skills", isDirectory: true)
    }
    ...
    /// Resolves `~/.carbonara/skills/<id>/` only when `id` is a single safe path component.
    ...
    /// Copies under a `carbonara-` prefix so we only overwrite our own prior copy
    @discardableResult
    func copy(_ skill: Skill, to agent: SkillExternalAgent) -> URL? {
        let source = skill.path.deletingLastPathComponent()
        let dest = agent.skillsDirectory.appendingPathComponent("carbonara-\(skill.id)", isDirectory: true)
        let fm = FileManager.default
        do {
