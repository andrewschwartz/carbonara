...
extension ToolExecutor {
    func manageExports(_ editor: EditorViewModel, _ args: [String: Any]) throws -> ToolResult {
        ...
        case "list":
            ...
            let exports = projectJobs.reversed().map { job -> [String: Any] in
                ...
                if let report = job.projectPackageReport {
    ...
    func exportProject(_ editor: EditorViewModel, _ args: [String: Any]) async throws -> ToolResult {
        ...
        if let id = input.timelineId {
            guard mode != .package else {
                throw ToolError("export_project: timelineId doesn't apply to package mode — the package carries every timeline")
            }
        ...
        case .package:
            return try enqueueProjectPackageExportTool(editor, outputURL: outputURL)
        }
    }
    ...
    private func enqueueProjectPackageExportTool(_ editor: EditorViewModel, outputURL: URL) throws -> ToolResult {
        let submission = try exportQueue.enqueueProjectPackageExport(
        ...
        let payload: [String: Any] = [
            ...
            "jobId": submission.jobID.uuidString,
            "queuePosition": submission.queuePosition,
            "mode": ExportProjectMode.package.rawValue,
            "path": outputURL.path,
            "note": "The export is available in the Export dialog.",
        ]
        return try jsonResult(payload)
    ...
    private func directExportURL(_ path: String, mode: ExportProjectMode, format: ExportFormat?) throws -> URL {
        ...
        if fm.fileExists(atPath: expanded, isDirectory: &isDirectory),
           ...
           !(mode == .package && rawExtension == expectedExtension) {
...
private extension ProjectPackageExporter.Report {
...
private enum ExportProjectMode: String {
    ...
    init(named raw: String?) throws {
        ...
        guard let mode = Self(rawValue: raw.normalizedExportOption) else {
            throw ToolError("export_project: mode must be video, xml, fcpxml, or package")
        }
        self = mode
    }
    ...
    func fileExtension(format: ExportFormat?) -> String {
        ...
        case .xml: "xml"
        case .fcpxml: "fcpxml"
        case .package: Project.fileExtension
        }
    }
    ...
    func label(format: ExportFormat?) -> String {
        ...
        case .xml: "XML"
        case .fcpxml: "FCPXML"
        case .package: "Carbonara Project"
        }
    }
}
