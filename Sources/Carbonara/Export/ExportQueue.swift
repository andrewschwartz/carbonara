...
struct ExportJob: Identifiable, Sendable {
    ...
    var error: String?
    var warnings: [String]
    var projectPackageReport: ProjectPackageExporter.Report?
}
...
final class ExportQueue {
    ...
    @discardableResult
    func enqueueProjectPackageExport(
        ...
        try enqueue(outputURL: outputURL, projectID: projectID, source: source) { service in
            await service.exportProjectPackage(
                projectFile: projectFile,
                manifest: manifest,
                generationLog: generationLog,
    ...
    private func finish(_ id: UUID, status: ExportJobStatus, service: ExportService) {
        ...
        if let report = service.lastProjectPackageReport {
            jobs[index].projectPackageReport = report
...
private extension ExportJob {
    func warningCount(_ mediaReport: ExportRunReport?) -> Int {
        if let projectPackageReport { return projectPackageReport.missing.count }
        if let mediaReport {
