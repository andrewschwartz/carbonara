...
private struct ExportAnalyticsRun {
    ...
    init(packageContext context: ExportAnalyticsContext) {
        self.basePayload = [
            "source": context.source,
            "project_id": context.projectId ?? "unknown",
            "mode": "package",
...
final class ExportService {
    ...
    var lastProjectPackageReport: ProjectPackageExporter.Report?
    ...
    func exportProjectPackage(
        ...
        reset()
        defer { activeCancellation = nil }
        let analytics = ExportAnalyticsRun(packageContext: analyticsContext)
        ...
        do {
            ...
            setPhase(.exporting)
            Log.export.notice(
                "package export start url=\(outputURL.lastPathComponent)",
                telemetry: "Carbonara project export started",
                data: [
            ...
            lastProjectPackageReport = report
            didCommitOutput = true
            setProgress(1)
            Log.export.notice(
                "package export ok collected=\(report.collected.count) missing=\(report.missing.count)",
                telemetry: "Carbonara project export finished",
            ...
            if Self.isCancellation(error) {
                wasCancelled = true
                Log.export.notice("package export cancelled", telemetry: "Export cancelled")
            } else {
                self.error = Log.detail(error)
                Log.export.error(
                    "package export failed: \(Log.detail(error))",
                    telemetry: "Carbonara project export failed",
