...
enum ExportDestination: String, CaseIterable, Identifiable {
    ...
    case projectPackage = "Carbonara Project"
...
struct ExportView: View {
    ...
    var body: some View {
        ...
        .task {
            ...
            let summary = await Task.detached(priority: .utility) {
                Self.computeProjectPackageSummary(entries: entries, projectURL: projectURL)
            }.value
        ...
        VStack(spacing: AppTheme.Spacing.zero) {
            VStack(alignment: .leading, spacing: AppTheme.Spacing.zero) {
                VStack(alignment: .leading, spacing: AppTheme.Spacing.zero) {
                    ...
                    if editor.timelines.count > 1, destination != .projectPackage {
    ...
    private var projectPackageSettings: some View {
        VStack(alignment: .leading, spacing: AppTheme.Spacing.xs) {
            ...
            if packageSummary.missing > 0 {
                Text("\(packageSummary.missing) media file\(packageSummary.missing == 1 ? "" : "s") missing - they'll be skipped.")
                    .font(.system(size: AppTheme.FontSize.xs))
                    .foregroundStyle(AppTheme.Status.errorColor)
            }
        }
        ...
        return HStack(spacing: AppTheme.Spacing.lg) {
            ...
            case .projectPackage:
                HStack(spacing: AppTheme.Spacing.xs) {
                    ...
                    Text("~\(ByteCountFormatter.string(fromByteCount: packageSummary.bytes, countStyle: .file))")
                }
    ...
    private var exportFormat: ExportFormat {
        ...
        case .projectPackage: .xml   // Carbonara Project has its own path; never rendered.
        case .video: codec.exportFormat
        }
    }

    /// Quick estimate for exporting a Carbonara Project
    private nonisolated static func computeProjectPackageSummary(
    ...
    private func startExport() {
        if destination == .projectPackage { startProjectPackageExport(); return }
        submissionError = nil
    ...
    private func startProjectPackageExport() {
        ...
        panel.begin { response in
            guard response == .OK, let url = panel.url else { return }
            do {
                try exportQueue.enqueueProjectPackageExport(
