...
final class GenerationService {
    ...
    func generate(
        ...
        let provider = ProviderRouter.provider(for: genInput.model)
    ...
    func resumePendingGenerations(editor: EditorViewModel) {
        ...
        for (backendJobId, group) in byBackendJob where !resumedBackendJobIds.contains(backendJobId) {
            ...
            let provider = ProviderRouter.provider(for: placeholders.first?.generationInput?.model ?? "")
    ...
    private func runJob(
        ...
        Log.generation.notice("run \(runId) start provider=\(provider.id) model=\(genInput.model) placeholders=\(placeholders.count)")
    ...
    private func monitorBackendJob(
        ...
        var sawUpdate = false
        for await update in provider.updates(forJob: backendJobId) {
            sawUpdate = true
            if await applyProviderUpdate(
                update: update,
                backendJobId: backendJobId,
                placeholders: placeholders,
                editor: editor,
        ...
        // Provider unavailable: no update ever arrived and this run demanded one.
        if !sawUpdate, failIfUnavailable {
            for placeholder in placeholders {
                updateGenerationMetadata(placeholder, editor: editor, status: .failed("Backend not available"))
            }
            editor.onProjectCheckpointRequired?()
            onFailure?()
            return
    ...
    private func applyProviderUpdate(
        ...
        case .succeeded:
            ...
            return true
        case .failed:
            let message = update.errorMessage ?? "Generation failed"
