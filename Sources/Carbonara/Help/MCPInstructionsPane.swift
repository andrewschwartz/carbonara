...
struct MCPInstructionsPane: View {
    ...
    private var claudeCodeCommand: String {
        "claude mcp add --transport http carbonara \(mcpEndpoint)"
    }

    private var codexCommand: String {
        "codex mcp add carbonara --url \(mcpEndpoint)"
    }
    ...
    private var cursorDeepLink: URL? {
        ...
        return URL(string: "cursor://anysphere.cursor-deeplink/mcp/install?name=carbonara&config=\(encoded)")
    }

    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: AppTheme.Spacing.xxl) {
                Text("Connect an external agent to inspect and edit the open Carbonara project.")
    ...
    private var cursorSection: some View {
        agentSection(
            .cursor,
            name: "Cursor",
            description: "Install the Carbonara MCP server in Cursor.",
    ...
    private var claudeDesktopSection: some View {
        agentSection(
            .claude,
            name: "Claude Desktop",
            description: "Install the bundled Carbonara connector.",
    ...
    private func openClaudeDesktopBundle() {
        guard let bundleURL = claudeDesktopBundleURL else {
            claudeInstallError = "The Carbonara connector could not be found. Reinstall Carbonara, then try again."
            return
        }
        ...
        NSWorkspace.shared.open(
            ...
            Task { @MainActor in
                claudeInstallError = "Claude Desktop could not open the Carbonara connector. Update Claude Desktop, then try again."
            }
        }
    }

    private var claudeDesktopBundleURL: URL? {
        BundledResource.url("carbonara.mcpb")
    }
}
